/**
 * Approximate the straight-line distance (Haversine) in meters
 */
export function calculateDistanceMeters(lat1, lng1, lat2, lng2) {
	...
	/**
	 * Determine the a/b values for the distance range we fall into.
	 * distance_ranges is an array of objects [{min, max, a, b, ...}, ...].
	 *
	...
	export function findDistanceRange(distance, distanceRanges) {
		for (const range of distanceRanges) {
			// Per the problem statement, "max: 0" means no delivery for distance >= min
			if (range.max === 0) {
	 ...
	 */
	export function calculateDeliveryFee(basePrice, a, b, distance) {
	 ...
	 */
	export function calculateSmallOrderSurcharge(cartValue, minNoSurcharge) {
